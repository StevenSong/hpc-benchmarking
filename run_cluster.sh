...
if [ $# -lt 4 ]; then
    echo "Usage: $0 docker_image head_node_ip --head|--worker path_to_downloaded_models [additional_args...]"
    exit 1
fi
...
# -v DOWNLOADED_MODELS: Mounts downloaded models
docker run \
    --entrypoint /bin/bash \
    --network host \
    --name "${CONTAINER_NAME}" \
